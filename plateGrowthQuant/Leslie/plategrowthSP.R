...
fx.file=paste0(ComplexTraitToolsPath, 'plateQTL/plategrowthSPFx.R') 

#base directory for sub-experiment
...
key.file = paste(layout.path, 'Key2.csv', sep='')
...
#dumb munging stuff for WI collection ---------------------------------------------------------------
    ...
    StrainName=strain.id[,2] #gsub('(\\d{1-3}):(.*)', '\\2' , strain.id[,1])
