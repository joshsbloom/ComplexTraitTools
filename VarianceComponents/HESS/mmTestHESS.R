...
doTraitFDR=function(trait, genos, genos.full, FDR_thresh=.05, nperm=1e4) {
    f.found=c()
    p.found=c()
    q.found=c()
    m.found=c()
...
#    print(i)
#    svd.V=svd(lcor[[i]])
#    cutoff=cumsum(svd.V$d)/sum(svd.V$d)<.95
#    qs[i]=sum(cutoff) 
#    VinvL[[i]]=svd.V$v[,cutoff] %*% diag(1/svd.V$d[cutoff]) %*% t(svd.V$u[,cutoff])
#    qs[i]=cutoff
#}

V=crossprod(gdata.s)/(nrow(gdata.s)-1)
svd.V=svd(V)
cutoff=cumsum(svd.V$d)/sum(svd.V$d)<.9999
q=sum(cutoff) 
Vinv=svd.V$v[,cutoff] %*% diag(1/svd.V$d[cutoff]) %*% t(svd.V$u[,cutoff])
LDS=apply(V,1,function(x) sum(x^2))
...
for( k in 1:100) {
    ...
    y=scale(as.vector(y))
    #test=doTraitFDR(y,gdata.s, FDR_thresh=.05, nperm=1e3)
    Betas=cor(y,gdata.s)[1,]
    B=as.matrix(Betas)
    #HESS
    h2e[k]=(N*(t(B) %*% Vinv %*% (B)) - q)/(N-q)
    
    #LD SCORE regression ------------------------
    #nullik=logLik(lm(y~1))
    #flik=rep(0,ncol(gdata.s))
    #for(i in 1:ncol(gdata.s)){
    #    flik[i]=logLik(lm(y~gdata.s[,i]))
    ##     print(i)
    #}
    #chisq=-2*(nullik-flik) #nullik-flik
    Bsq=Betas^2
    #lm(chisq~(Bsq)-1)
    chisqe=Bsq*1032
    mf=lm(chisqe~LDS)
    ...
    #rr=regress(y~1, ~A)$sigma
    #h2a[k]=rr/sum(rr)
    print(k)
}


BL=rr$sigma[1]*t(gdata.s)%*%rr$W %*%(as.matrix(y))


    par(mfrow=c(2,1))
    hist(h2a, xlim=c(0.6,1), main='reml n=1008, h2=.8, 40 QTL')
    hist(h2e, xlim=c(0.6,1), main='hess n=1008, h2=.8, 40 QTL' )

    
    Beta.c=split(Betas,chr)
    ...
    h2e=rep(NA,16) 
    for(i in 1:16) {
        B=as.matrix(Beta.c[[i]])
        h2e[i]=(N*(t(B) %*% VinvL[[i]] %*% B) - qs[i])/(N-qs[i])
    }
}
