...
nsample=1008
nmarker=11623
nadditive=500 #c(0,1,5,10,50,500)
#nint=c(0,1,5,10,50,500)
#H2=seq(0,.8, .2)
H2=.75
#h2=seq(0,.75, .15)
h2=.75
...
# get half
    add.qtl.ind  = (add.qtl.ind)[1:250]
add.qtl.sign = sample(c(1,-1),nadditive,replace=T)
    add.qtl.sign=-add.qtl.sign[1:250]
#add.qtl.sign=add.qtl.sign[1:250]
#for assigning QTL
    nadditive=250 #c(0,1,5,10,50,500)

if(nadditive>0) { 
    for(i in 1:nadditive){ a.eff=a.eff+add.qtl.sign[i]*gdata[,add.qtl.ind[i]] } 
}
if(nadditive>0) { a.eff=scale(a.eff) }
