...
efitness=extraDistr::rinvchisq(ncol(g),12,1)
hist(efitness)
eCount=rowSums(bg%*%efitness)
...
#Gy=crossprod(g, t1)
Gy0=crossprod(bg[1:nrow(g),], t1[1:nrow(g),])
Gy=crossprod(bg, t1)
...
predictions0=apply(Gy0,2,function(x) {
       as.vector(RcppML::nnls(GGp0,matrix(x), fast_nnls=T)) #fast_nnls=F, cd_maxit=10000, cd_tol=1e-10))

})
predictions0=apply(predictions0, 2, function(x) x/sum(x))

#dependencies are abind and RcppML
...
#construct the bootstrap matrices 
for(n in 1:nboot) {
    print(n)
    nind=sample(1:nrow(g), replace=T)
    bootGy[[as.character(n)]]=crossprod(g[nind,],t1[nind,])
    bootGGp[[as.character(n)]]=crossprod(g[nind,])
}

# for each bootstrap calculate strain frequency
blist=list()
for(n in 1:nboot) {
    print(n)
    bGy=bootGy[[as.character(n)]]
    bGGp=bootGGp[[as.character(n)]]

    bpredictions=apply(bGy,2,function(x) {
       as.vector(RcppML::nnls(bGGp,matrix(x), fast_nnls=T)) #fast_nnls=F, cd_maxit=10000, cd_tol=1e-10))

     })
    bpredictions=apply(bpredictions, 2, function(x) x/sum(x))
    blist[[as.character(n)]]=bpredictions
}

# turn into a 3d array
ab=abind::abind(blist, along=3)

# extract bootstrap SE 
bootse=apply(ab, c(1,2), function(x) sd(x))

#library(gplots)

par(mfrow=c(2,3))
for(i in 1:ncol(predictions)){
    gplots::plotCI(predictions[,i], (efitness.norm), err='x', uiw=1.96*bootse[,i], sfrac=.005,
